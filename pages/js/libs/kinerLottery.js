/**
 * 注意：本插件运用了rem屏幕适配方案，一律采用rem作为单位，若项目中不是采用这种方案的，
 * 请在kinerTreeMenu.css中修改样式，此段代码不会影响功能使用，仅会影响控件样式
 */
	...
	KinerLottery.prototype.init = function () {
		...
		$('body').on('click', ".KinerLotteryBtn", function () {
			...
			if ($(this).hasClass('start') && !self.doing) {
				...
				var key = $(this).hasClass('no-start') ? "noStart" :
					$(this).hasClass('completed') ? "completed" : "illegal";
		...
		////转动结束
		//$(this.opts.body).find('.KinerLotteryContent').get(0).webkitTransitionEnd(function () {
		//	self.doing = false;
		//
		...
		//转动结束
		$(this.opts.body).find('.KinerLotteryContent').get(0)
